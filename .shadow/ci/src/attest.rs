...
//
// For an audit window: build the POPULATION of changes from three independent
// sources (GitHub's merged-PR list, the git commit history of staging, and the
...
//   T2  reviewed:    review activity from an identity other than the author
//                    (human review, or the shadow-review marker)
...
//   T5  flow:        the PR targeted staging (main-based PRs are hotfix-path
//                    and flagged for incident verification)
//
...
pub fn run_attest() -> Result<i32, String> {
    ...
    let since = std::env::var("SINCE").map_err(|_| "SINCE required (ISO date, start of audit window)")?;
    ...
    let prs_raw = gh_json(&[
        ...
        "--json", "number,title,body,author,createdAt,mergedAt,mergedBy,baseRefName,reviews",
    ])?;
            ...
            a.iter()
                .filter(|p| {
                    in_window(p["mergedAt"].as_str().unwrap_or(""), &since, &until)
                        && matches!(p["baseRefName"].as_str(), Some("staging") | Some("main"))
                })
                .collect()
        })
        .unwrap_or_default();

    // Source B — the git history of staging (what actually landed)
    git(&["fetch", "-q", "origin", "staging"]).ok();
    let commits = git(&[
        "log", "origin/staging", &format!("--since={since}"), &format!("--until={until}"),
        "--pretty=%H|%cI|%s",
    ])
    .unwrap_or_default();
    ...
    let archive_files = git(&["ls-tree", "-r", "--name-only", &format!("origin/{archives}")]).unwrap_or_default();
    ...
    for p in &prs {
        ...
        // T2 reviewed — someone other than the author (or the shadow reviewer's marker)
        let human_review = p["reviews"]
            .as_array()
            .map(|a| a.iter().any(|r| r["author"]["login"].as_str().unwrap_or("") != author))
            .unwrap_or(false);
        let marker_review = gh_json(&["api", &format!("repos/{repo}/issues/{n}/comments"), "--paginate"])
            .map(|c| crate::check::shadow_review_posted(&c))
            .unwrap_or(false);
        if !human_review && !marker_review {
            fails.push("T2: no independent review activity found".into());
        }

        // T3 gated — the archive's bypass analysis
        if archived_prs.contains(&n) {
            if let Some(file) = archive_files.lines().find(|f| {
                f.ends_with(".json") && archive_pr_number(f) == Some(n)
            }) {
        ...
        // T5 flow
        if base == "main" {
            fails.push("T5: PR targeted main directly (hotfix path — verify incident ticket)".into());
        }
    ...
    for c in &orphan_commits {
        exceptions.push(format!("staging commit without a PR (direct push): {c}"));
    }
    ...
    for n in &archived_not_prs {
        exceptions.push(format!("archive record pr-{n} has no matching merged PR — investigate"));
    }

    let tested = items.len();
    let passed = items.iter().filter(|i| i["pass"].as_bool() == Some(true)).count();
    ...
    let report = json!({
        "attestation": "change management (CC8.1, CC6.3-review, CC4.1)",
        "repo": repo, "window": {"since": since, "until": until},
        "method": "population from three independent sources (GitHub merged-PR API, git history of staging, compliance-archives), reconciled; 100% attribute testing (no sampling)",
        "population": {
            "merged_prs": pr_numbers.len(),
            "staging_commits_without_pr": commits_without_pr,
        ...
        "attributes": ["T1 authorized (ticket precedes change)", "T2 independently reviewed", "T3 gated (no bypass)", "T4 documented (4 sections)", "T5 flow (via staging)"],
        "items": items,
    ...
    let files = vec![
        (format!("evidence/attestations/attestation-{stamp}.json"), serde_json::to_string_pretty(&report).unwrap()),
        (format!("evidence/attestations/attestation-{stamp}.md"), md.clone()),
    ];
    if env_or("ARCHIVES_PUSH", "") == "1" {
        crate::util::commit_to_archives(&archives, &files, &format!("evidence: attestation {stamp}"))?;
        println!("attestation pushed to {archives}");
    } else {
        std::fs::write(format!("attestation-{stamp}.md"), &md).map_err(|e| e.to_string())?;
        std::fs::write(format!("attestation-{stamp}.json"), serde_json::to_string_pretty(&report).unwrap())
...
fn render_md(r: &Value) -> String {
    ...
    let p = &r["population"];
    s.push_str(&format!(
        "## Population & completeness\n\n| Measure | Value |\n|---|---|\n| Merged PRs (staging+main) | {} |\n| Staging commits with no PR (direct pushes) | {} |\n| PRs missing from commit history | {:?} |\n| PRs without an archive record | {:?} |\n| Archive records without a PR | {:?} |\n\n",
        p["merged_prs"], p["staging_commits_without_pr"], p["prs_not_in_commit_history"], p["prs_without_archive_record"], p["archive_records_without_pr"],
    ));
    ...
    for i in r["items"].as_array().unwrap_or(&vec![]) {
        s.push_str(&format!(
            "| #{} | {} | {} | {} | {} |\n",
            i["pr"],
            i["title"].as_str().unwrap_or("").chars().take(60).collect::<String>(),
            i["author"].as_str().unwrap_or(""),
            i["merged_by"].as_str().unwrap_or(""),
            if i["pass"].as_bool() == Some(true) { "✓" } else { "✗ see exceptions" },
        ));
    }
    ...
    s.push_str(&format!("\n---\nGenerated {} by `shadow-ci attest` from GitHub's own records. Attributes: T1 authorized (ticket precedes change) · T2 independently reviewed · T3 gated, no bypass · T4 documented · T5 via staging.\n", r["generated_at"].as_str().unwrap_or("")));
    s
}
...
mod tests {
    ...
    fn pr_numbers_from_merge_subjects() {
        ...
        assert_eq!(pr_number_from_subject("Merge pull request #7 from x/y"), Some(7));
    ...
    fn window_and_archive_parsing() {
        assert!(in_window("2026-05-02T10:00:00Z", "2026-05-01", "2026-07-31"));
        assert!(!in_window("2026-04-30T23:59:59Z", "2026-05-01", "2026-07-31"));
    ...
    fn documentation_attribute() {
        ...
        assert_eq!(missing_sections("## Summary only"), vec!["## Tickets", "## Changes", "## Test Plan"]);
    }
}
